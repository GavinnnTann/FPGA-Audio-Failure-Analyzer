...
Magnitude Extraction (Alphamax+Betamax, first 256 bins only)
  ↓
Mel Filterbank (256 linear bins → 64 mel-spaced bands, 0-8 kHz, Slaney scale)
  ↓
UART Burst: 1 RMS frame + 64 spectrogram slice packets
```

**Frequency coverage**: 0 Hz to 8 kHz, mel-spaced (not linear) — matches the CNN autoencoder's
training-time feature extraction (`librosa.feature.melspectrogram` in `submission/src/audio.ipynb`).
Bins above ~8 kHz carry zero energy by construction; the FFT's own Nyquist is ~23.4 kHz but content
above 8 kHz is out of scope for the mel bands the CNN was trained on.
**Band spacing**: non-linear — dense at low frequency, widening at high frequency (see table below).
Exact center frequencies are generated by `scripts/gen_mel_coeffs.py` and printed to console when run.
...
**Bin Index → Mel Band Center Frequency** (non-linear; regenerate via `scripts/gen_mel_coeffs.py` if
filterbank parameters change):
- `bin_index = 0`: ~46 Hz
- `bin_index = 1`: ~93 Hz
- `bin_index = 10`: ~511 Hz
- `bin_index = 27`: ~1.36 kHz
- `bin_index = 41`: ~2.66 kHz
- `bin_index = 63`: ~7.63 kHz

There is no closed-form linear formula — each band's center frequency comes from the Slaney
mel-scale filter design (`hz_to_mel`/`mel_to_hz` in `scripts/gen_mel_coeffs.py`), evenly spaced in
*mel* space rather than Hz space.
...
**Magnitude extraction** (fft_magnitude.v) — no decimation, all 256 linear bins pass through:
```verilog
// Only first 256 bins (real FFT); bin reduction now happens in mel_filterbank.v
wire is_valid_bin = (fft_bin_index < 10'd256);
```

**Mel filterbank** (mel_filterbank.v) — 256 linear bins → 64 mel bands:
```verilog
// Two (band, weight) contributions per linear bin, looked up from mel_coeffs.mem
// (contiguous triangular filters overlap only their immediate neighbour)
wire [5:0] mel_a = coeff[13:8];  wire [7:0] w_a = coeff[7:0];
wire [5:0] mel_b = coeff[29:24]; wire [7:0] w_b = coeff[23:16];
```
...
void redraw_spectrogram_display() {
    // 64 mel-spaced bins spanning 0 to ~8 kHz (non-linear spacing —
    // dense at low frequency, widening at high frequency)
    ...
    // Frequency labels (mel-spaced, non-linear — see scripts/gen_mel_coeffs.py):
    // "~46 Hz" at bin 0
    // "~1.7 kHz" at bin 32
    // "~7.6 kHz" at bin 63
}
...
- FFT Output Format: [fft_magnitude.v](./src_main/fft_magnitude.v) (16-bit magnitude, first 256 bins, no decimation)
- Mel Filterbank: [mel_filterbank.v](./src_main/mel_filterbank.v) (256 linear bins -> 64 mel bands, coefficients from [scripts/gen_mel_coeffs.py](./scripts/gen_mel_coeffs.py))
